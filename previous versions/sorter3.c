...
*/
	...
	char line[500];
  	...
  	int total_num_of_movies=0, movie_number=0,track_tokenizer=0;
   ...
   /* while (fgets(line,500,fileName)){
    ...
    // in line you have the the line , you can do whatever you want
    //token = strtok(line, ",");
    i = subTokenize(line, token,i);
    printf("%s\n", token);

        if (strlen(token)> strlen(color)){
         strcpy(color, token);
        }
       ...
       printf("%s\n", token);
        ...
        if (strlen(token)> strlen(director_name)){
         strcpy(director_name,token);
        }

        //director to facebook like and the next one //
